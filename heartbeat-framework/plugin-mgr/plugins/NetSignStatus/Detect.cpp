...
void makeMessage(unsigned char *pmsg, long msglen, unsigned char identifier, const char *type, unsigned char *pOutbuf,
                 long *outMesLen)
{
    char tempbuf[20] = {0};
...
void
makeMessageHead(unsigned char *pmsg, long msglen, unsigned char identifier, const char *type, unsigned char *pOutbuf,
                long *outMesLen)
{
    char tempbuf[20] = {0};
    memcpy(pOutbuf, (unsigned char *) &identifier, TAGNAMELEN);
...
int receiveTagAndLen(int sockFd, unsigned char *TAG, int *iSegmentLen, int *iHaveReceiveLen)
{
    ...
    iCurrentReceiveLen = receiveContent(sockFd, TAGNAMELEN + TAGCLASSLEN + COMMLENGTHLEN, (unsigned char *) tmpBuffer,
                                        iHaveReceiveLen);
...
int handleResult(unsigned char TAG, char *tmpBuffer, int iReadLen, int handleType, int *iResult, result_param *result,
                 const char *info)
{
